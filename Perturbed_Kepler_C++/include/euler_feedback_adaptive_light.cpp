...
// ----- EL-feedback RHS (kL,kE는 피드백 계수 전용) -----
// xdot = v - kE*dE*Ur(r)*x/r - kL*(v × dL)
// vdot = a(x) - kE*dE*v      - kL*(dL × x)
static std::array<double,6> fi_rhs_EL(const std::array<double,6>& z,
                                      const PKParams& P,
                                      double kL_eff,
                                      double kE_eff,
                                      ...
                                      double E0)
{
    const double x[3] = { z[0], z[1], z[2] };
    const double v[3] = { z[3], z[4], z[5] };
    const double r = std::max(1e-15, norm3(x));

    double L[3]; cross3(x, v, L);
    double dL[3] = { L[0]-L0[0], L[1]-L0[1], L[2]-L0[2] };
    const double E = 0.5*(v[0]*v[0]+v[1]*v[1]+v[2]*v[2]) + U_scalar(r,P);
    ...
    double xdot[3] = {
        v[0] - kE_eff*dE*upr_over_r*x[0] - kL_eff*vXdL[0],
        v[1] - kE_eff*dE*upr_over_r*x[1] - kL_eff*vXdL[1],
        v[2] - kE_eff*dE*upr_over_r*x[2] - kL_eff*vXdL[2]
    };

    double ax[3]; accel(x,P,ax);
    double dLxX[3]; cross3(dL, x, dLxX);
    double vdot[3] = {
        ax[0] - kE_eff*dE*v[0] - kL_eff*dLxX[0],
        ax[1] - kE_eff*dE*v[1] - kL_eff*dLxX[1],
        ax[2] - kE_eff*dE*v[2] - kL_eff*dLxX[2]
    };

    return { xdot[0], xdot[1], xdot[2], vdot[0], vdot[1], vdot[2] };
}

static inline std::array<double,3> cross_init(double x1,double x2,double x3,
                        ...
                        const double Hmin)
{
    ...
    // L0, E0 내부 계산
    const std::array<double,3> L0 = cross_init(x1[0],x2[0],x3[0], v1[0],v2[0],v3[0]);
    const double r0 = std::max(1e-15, std::sqrt(x1[0]*x1[0]+x2[0]*x2[0]+x3[0]*x3[0]));
    const double v0sq = (v1[0]*v1[0]+v2[0]*v2[0]+v3[0]*v3[0]);
    const double E0 = 0.5*v0sq + U_scalar(r0,P);

    // 첫 Lipschitz 근사 및 α
    double Lhat = max(Hmin, estimateL(current_state, P, kL, kE)); // Frobenius approx
    double alpha = 1.0 / ( h * lambda * Lhat );
    ...
    for(int i = 0; i < N; ++i)
    {
        ...
        auto F = fi_rhs_EL(current_state, P, kL_eff, kE_eff, L0, E0);

        std::array<double,6> next_state;
        for(int j=0; j<6; j++) next_state[j] = current_state[j] + h*F[j];
                                        ...
                                        const double Hmin)
{
    ...
    array<double,6> current_state = {x1, x2, x3, v1, v2, v3};
    double current_time = 0.0;

    // L0, E0 내부 계산
    const std::array<double,3> L0 = cross_init(xi[0],xi[1],xi[2], xi[3],xi[4],xi[5]);
    const double r0 = std::max(1e-15, std::sqrt(xi[0]*xi[0]+xi[1]*xi[1]+xi[2]*xi[2]));
    const double v0sq = (xi[3]*xi[3]+xi[4]*xi[4]+xi[5]*xi[5]);
    const double E0 = 0.5*v0sq + U_scalar(r0,P);
